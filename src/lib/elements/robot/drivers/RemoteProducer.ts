...
export class RemoteProducer implements Producer {
	...
	async connect(joinExistingRoom = false): Promise<void> {
		try {
			...
			console.log(
				`[RemoteProducer] Connecting to ${serverUrl} for robot ${this.config.robotId} (join mode: ${joinExistingRoom})`
			);
			...
			if (joinExistingRoom) {
				...
				console.log(
					`[RemoteProducer] Joining existing room ${this.config.robotId} in workspace ${this.workspaceId}`
				);
			} else {
				// Create new room (for standalone operation)
				roomData = await this.client.createRoom(this.workspaceId, this.config.robotId);
				console.log(
					`[RemoteProducer] Created new room ${roomData.roomId} in workspace ${roomData.workspaceId}`
				);
			}
			...
			console.log(
				`[RemoteProducer] Connected successfully to workspace ${roomData.workspaceId}, room ${roomData.roomId}`
			);
		} catch (error) {
	...
	private startStateUpdates(): void {
		...
		this.stateUpdateInterval = setInterval(async () => {
			if (
				...
				Object.keys(this.lastKnownState).length > 0
			) {
				try {
