...
export class RemoteConsumer implements Consumer {
	...
	async connect(joinExistingRoom = false): Promise<void> {
		try {
			...
			console.log(
				`[RemoteConsumer] Connecting to ${serverUrl} for robot ${this.config.robotId} (join mode: ${joinExistingRoom})`
			);
			...
			if (joinExistingRoom) {
				...
				console.log(
					`[RemoteConsumer] Joining existing room ${this.config.robotId} in workspace ${this.workspaceId}`
				);
			} else {
				// Create new room (for standalone operation)
				roomData = await this.client.createRoom(this.workspaceId, this.config.robotId);
				console.log(
					`[RemoteConsumer] Created new room ${roomData.roomId} in workspace ${roomData.workspaceId}`
				);
			}
			...
			console.log(
				`[RemoteConsumer] Connected successfully to workspace ${roomData.workspaceId}, room ${roomData.roomId}`
			);
		} catch (error) {
